...
public class PlanningService implements IService<Planning> {
    ...
    public void Add(Planning planning) {
        try{
            ...
            req.setDate(1,new java.sql.Date(planning.getdateDebut().getTime()));
            req.setString(2,planning.getDescription());
            req.setString(3,planning.getTitle());
            req.setDate(4,new java.sql.Date(planning.getdateFin().getTime()));
            req.executeUpdate();
    ...
    public void Update(Planning planning) {
        try{
            PreparedStatement req=con.prepareStatement ("UPDATE `planning` SET dateDebut=?,description=?,title=?,dateFin=? WHERE id=?");
            req.setDate(1,new java.sql.Date(planning.getdateDebut().getTime()));
            req.setString(2,planning.getDescription());
            req.setString(3,planning.getTitle());
            req.setDate(4,new java.sql.Date(planning.getdateFin().getTime()));
            req.setInt(5,planning.getId());
            req.executeUpdate();
