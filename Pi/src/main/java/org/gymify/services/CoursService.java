...
public class CoursService implements IService<Cours> {
    ...
    public void Add(Cours cours) {
        try {
            PreparedStatement req = con.prepareStatement(" INSERT INTO `cours`(title, description, dateDebut, dateFin, activité_id, planning_id) VALUES (?,?,?,?,?,?)");
            ...
            req.setInt(5,cours.getActivité().getId());
            req.setInt(6,cours.getPlanning().getId());
            req.executeUpdate();
    ...
    public void Update(Cours cours) {
        try {
            PreparedStatement req= con.prepareStatement("UPDATE cours SET title=?,description=?,dateDebut=?,dateFin=?,activité_id=?,planning_id=? WHERE id=?");
            req.setString(1, cours.getTitle());
            req.setString(2, cours.getDescription());
            req.setDate(3, new java.sql.Date(cours.getDateDebut().getTime()));
            req.setDate(4, new java.sql.Date(cours.getDateFin().getTime()));
